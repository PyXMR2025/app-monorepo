 ...
 *   - buildHardwareEvmTransaction + assembleHardwareSignedEvmTx wired in for
 *     both EIP-1559 and legacy shapes
...
// Hex tokens used across tests. Must be even-length hex — the keychain
// encoder fails fast on anything else (see encodeHexTokenToBuffer).
const MOCK_HEX_PS_FROM_KEYCHAIN = 'ab12cd34';
const MOCK_HEX_SID_FROM_KEYCHAIN = 'ef56ab78';
const MOCK_HEX_STALE_PS = 'ba11beef';
const MOCK_HEX_STALE_SID = 'facefeed';
const MOCK_HEX_RESOLVED_PS = 'deadbeefcafe1234';
...
function makeDeps(
  ...
  // passphraseState from the SDK is always a hex string (e.g. "abc12345");
  // the default here matches that contract so keychain encoding round-trips.
  const resolvePassphraseStateByMode =
    overrides.resolveByMode ?? jest.fn(async () => MOCK_HEX_RESOLVED_PS);
...
describe('SignerHardware', () => {
  ...
  describe('passphraseMode = on_host with warm keychain', () => {
    it('preloads session cache and reuses passphraseState from keychain', async () => {
      const { deps, mocks } = makeDeps({
        keychainEntries: {
          [KEYCHAIN_PASSPHRASE_STATE_KEY]: Buffer.from(
            MOCK_HEX_PS_FROM_KEYCHAIN,
            'hex',
          ),
          [KEYCHAIN_SESSION_ID_KEY]: Buffer.from(
            MOCK_HEX_SID_FROM_KEYCHAIN,
            'hex',
          ),
        },
      });
      ...
      expect(params).toMatchObject({
        passphraseState: MOCK_HEX_PS_FROM_KEYCHAIN,
        skipPassphraseCheck: true,
      });
    ...
    it('unlocks device, skips stale keychain session, resolves fresh, re-persists', async () => {
      const { deps, mocks } = makeDeps({
        ...
        keychainEntries: {
          ...
          [KEYCHAIN_SESSION_ID_KEY]: Buffer.from(MOCK_HEX_STALE_SID, 'hex'),
        },
      });
      ...
      // Keychain now stores the raw hex bytes (one level of hex via
      // `security -i` / libsecret), not hex-of-utf8-of-hex.
      expect(mocks.keychainSet).toHaveBeenCalledWith(
        KEYCHAIN_PASSPHRASE_STATE_KEY,
        Buffer.from(MOCK_HEX_RESOLVED_PS, 'hex'),
      );
      expect(mocks.sdk.evmGetAddress.mock.calls[0][2]).toMatchObject({
        passphraseState: MOCK_HEX_RESOLVED_PS,
        skipPassphraseCheck: true,
      });
    });
  });
  ...
  describe('passphraseMode = on_host with empty keychain + unlocked device', () => {
    ...
    it('rejects a non-hex passphraseState from the SDK (fail-fast)', async () => {
      // Simulate a misbehaving SDK returning a non-hex token. Without
      // validation, `Buffer.from(x, 'hex')` silently drops the bad chars,
      // corrupting the keychain entry. We want a loud error instead.
      const { deps, mocks } = makeDeps({
        resolveByMode: jest.fn(async () => 'not-hex!!'),
      });
      const signer = new SignerHardware({
        device: DEVICE,
        passphraseMode: 'on_host',
      ...
      await expect(signer.getAddress('evm--1')).rejects.toThrow(
        /expected an even-length hex token/,
      );
      // Crucially: the corrupt token must NOT have been persisted to
      // keychain and must NOT have been forwarded to the SDK.
      expect(mocks.keychainSet).not.toHaveBeenCalled();
      expect(mocks.sdk.evmGetAddress).not.toHaveBeenCalled();
    });
