...
/**
 * Serialize a hex token (passphraseState / session_id from the SDK) into
 * the raw bytes that back it, so the keychain stores one layer of hex
 * (via `security -i` / libsecret) rather than hex-of-utf8-of-hex.
 *
 * Fails fast if the SDK ever returns a non-hex value — `Buffer.from(x,
 * 'hex')` would otherwise silently drop everything past the first
 * non-hex character, producing corrupt keychain entries that later
 * break `preloadSessionCache` with no obvious cause.
 */
export function encodeHexTokenToBuffer(hex: string): Buffer {
  if (!HEX_TOKEN_RE.test(hex) || hex.length === 0 || hex.length % 2 !== 0) {
    throw new OneKeyLocalError(
      `expected an even-length hex token, got length=${hex.length}: "${hex.slice(0, 8)}…"`,
    );
  }
  return Buffer.from(hex, 'hex');
}

/** Inverse of encodeHexTokenToBuffer — hex-encoded string restored from bytes. */
export function decodeHexTokenFromBuffer(buf: Buffer): string {
...
export abstract class SignerHardwareBase implements ISigner {
  ...
  protected async getHwCommonParams(): Promise<{
    ...
    if (!this.deviceWasLocked) {
      try {
        ...
        if (buf) {
          this.cachedPassphraseState = decodeHexTokenFromBuffer(buf);
          return { ...base, passphraseState: this.cachedPassphraseState };
        }
      } catch {
    ...
    if (fresh !== undefined) {
      // Validate upfront — a non-hex passphraseState from the SDK is a
      // bug we want to surface loudly (and avoid caching). Done outside
      // the best-effort try/catch below so it's not swallowed.
      encodeHexTokenToBuffer(fresh);
    ...
    if (this.cachedPassphraseState) {
      try {
        ...
        await keychain.set(
          KEYCHAIN_PASSPHRASE_STATE_KEY,
          encodeHexTokenToBuffer(this.cachedPassphraseState),
        );
  ...
  private async preloadSessionFromKeychain(): Promise<void> {
    ...
    try {
      ...
      if (psBuf && sidBuf) {
        const passphraseState = decodeHexTokenFromBuffer(psBuf);
        const sessionId = decodeHexTokenFromBuffer(sidBuf);
